...
export class Whitespace {
  ...
  constructor(config?: WhitespaceConfig) {
    ...
    if (config?.char !== undefined) this.chars = config.char
  }
...
export function Place(
  ...
  return PlaceVertical(height, vPos, PlaceHorizontal(width, hPos, str, config), config)
}
...
export function PlaceHorizontal(
  ...
  const ws = new Whitespace(config)
  const result: string[] = []
