...
import { type BorderStyle, borders, BorderType, getTopSize, getRightSize, getBottomSize, getLeftSize } from "./border"
...
export class Style {
  ...
  render(...strs: string[]): string {
    ...
    const teSpaceParts: string[] = []
    const teWhitespaceParts: string[] = []

    if (bold_) teParts.push(bold)
    if (italic_) teParts.push(italic)
    if (underline_) {
      switch (this._ul) {
        case "curly": teParts.push("\x1b[4:3m"); break
        case "dotted": teParts.push("\x1b[4:4m"); break
        case "dashed": teParts.push("\x1b[4:5m"); break
        case "double": teParts.push("\x1b[4:2m"); break
        default: teParts.push(underline)
      }
    }
    if (reverse_) { teParts.push(reverse); teWhitespaceParts.push(reverse) }
    if (blink_) teParts.push(blink)
    if (faint_) teParts.push(dim)
    if (strikethrough_) teParts.push(strikethrough)

    if (fgColor !== null) {
      const fgStr = fg(fgColor)
      teParts.push(fgStr)
      if (styleWhitespace) teWhitespaceParts.push(fgStr)
      if (useSpaceStyler) teSpaceParts.push(fgStr)
    }
    if (bgColor !== null) {
      const bgStr = bg(bgColor)
      teParts.push(bgStr)
      if (colorWhitespace || styleWhitespace) teWhitespaceParts.push(bgStr)
      if (useSpaceStyler) teSpaceParts.push(bgStr)
    }
    if (ulColor !== null) {
      const ulStr = ulColorFn(ulColor)
      teParts.push(ulStr)
      teWhitespaceParts.push(ulStr)
      if (useSpaceStyler) teSpaceParts.push(ulStr)
    }

    const te = teParts.join("")
    const teSpace = teSpaceParts.join("")
    const teWhitespace = teWhitespaceParts.join("")
      ...
      for (const line of lines) {
        if (line.length === 0) {
          ...
          for (const ch of line) {
            if (/\s/.test(ch)) {
              styledParts.push(teSpace, ch, reset)
            } else {
              styledParts.push(te, ch, reset)
            }
          }
          styledLines.push(styledParts.join(""))
        } else {
    ...
    if (this._link) {
      str = `\x1b]8;${this._linkParams};${this._link}\x07${str}\x1b]8;;\x07`
    }
    ...
    if (!this._inline) {
      ...
      if (this._paddingTop > 0) {
        const padLine = whitespaceStyle ? whitespaceStyle + padChar.repeat(getStringWidth(str)) + reset : ""
        str = Array(this._paddingTop).fill(padLine).join("\n") + (this._paddingTop ? "\n" : "") + str
      }
      if (this._paddingBottom > 0) {
        const padLine = whitespaceStyle ? whitespaceStyle + padChar.repeat(getStringWidth(str)) + reset : ""
        str += (this._paddingBottom ? "\n" : "") + Array(this._paddingBottom).fill(padLine).join("\n")
      }
    }
...
function styleBorderStr(border: string, fgColor: Color | null, bgColor: Color | null): string {
  ...
  if (fgColor) styleParts.push(fg(fgColor))
  if (bgColor) styleParts.push(bg(bgColor))
  return styleParts.join("") + border + reset
}
...
function styleBorderGradient(border: string, fgColors: string[], bgColor: Color | null): string {
  ...
  for (let i = 0; i < chars.length; i++) {
    const ch = chars[i]!
    const styleParts: string[] = []
    if (i < fgColors.length) styleParts.push(`\x1b[${colorToAnsi(fgColors[i]!, "38")}m`)
    if (bgColor) styleParts.push(bg(bgColor))
    outParts.push(styleParts.join(""), ch, reset)
  }
  return outParts.join("")
}
