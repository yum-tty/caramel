...
const bunColorCache = new Map<string, number[] | null>()

function colorCacheKey(c: Color): string {
...
/**
 * Parse a color string or value into a Color type.
 * @param s - Color string (#hex, number, or CSS name)
 * @returns Parsed Color or NoColor
 */
 ...
 * @inline - Frequently called, benefits from inlining
 * @param color - Color to convert
 * @param prefix - "38" for foreground, "48" for background, "58" for underline
...
export function colorToAnsi(color: Color, prefix: "38" | "48" | "58"): string {
  ...
  if (typeof c === "string" || (typeof c === "object" && c !== null && ("r" in c || Array.isArray(c)))) {
    const cacheKey = colorCacheKey(c)
    let rgba = bunColorCache.get(cacheKey)
    if (rgba === undefined) {
      rgba = Bun.color(c as any, "[rgba]") || null
      bunColorCache.set(cacheKey, rgba)
    }
    if (rgba) {
      return `${prefix};2;${rgba[0]};${rgba[1]};${rgba[2]}`
    }
    // Bun.color doesn't handle numeric strings like "63" — parse them as ANSI-256 indices
    if (typeof c === "string") {
